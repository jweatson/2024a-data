...
# Because this version takes in data from post-processed global and SNe data, 
local_data  = pd.read_pickle("all-sims-ratios.pkl.zst")
global_data = pd.read_pickle("global-model-virial-all-sim-ratios.pkl.zst")
sne_data    = pd.read_pickle("sne-model-all-sim-ratios.pkl.zst")

for i in tqdm(range(1,1000000)):
  # print(local_data.star[i])
  if local_data.star[i] != global_data.star[i]:
    print("FUCK")
  if local_data.star[i] != sne_data.star[i]:
    print("SHIT")
...
for ii,decay in enumerate(["decay","nodecay"]):
  ...
  for jj,model in enumerate(["global"]):
    use_tex()
    ...
    # Plot figure!
    outfilename = "violin-{}-{}-post-vir.pdf".format(model,decay)
