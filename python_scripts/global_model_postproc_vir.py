"""
Code to calculate 26Al and 60Fe yields using the global model from snapshots
"""
...
simulation_dirnames = sorted(glob("pt-*/pt-*-*/pt-*-*-fr-*/"))
...
for simulation_dirname in simulation_dirnames:
  ...
  yields_fname = sorted(glob(simulation_dirname+"*yields*ubj.zst"))[-1]
  ...
  wind_abs_nodiskdecay_26al_tot = np.zeros(len(first_cluster))
  wind_abs_nodiskdecay_60fe_tot = np.zeros(len(first_cluster))
  wind_abs_diskdecay_26al_tot  = np.zeros(len(first_cluster))
  wind_abs_diskdecay_60fe_tot  = np.zeros(len(first_cluster))
  ...
  # Create stellar evolution model for global model (determinate, so not an issue with accuracy)
  stellar = SeBa(number_of_workers=1)
  stellar.particles.add_particles(first_cluster)
  ...
  # Calculate yields for local model
  sim_yield = al26_plot.read_yields(yields_fname)
  lifetimes = first_cluster.tau_disk.value_in(units.myr)
  # Determine final values for each star in the case of decaying disks (achieved through interpolation)
  sim_yield = al26_plot.calc_disk_final_enrichment(sim_yield,lifetimes)
  
  # Progressively calculate values for global and sne model
  for i in tqdm(range(0,len(state_names)-1)):
    ...
    dt = t_next - t_curr
    # Calculate global model yields
    wind_abs_26al_curr, wind_abs_60fe_curr, stellar = al26_plot.calc_global_model_yield(curr_cluster,stellar,t_curr,dt,radius_method="virial")
    wind_abs_nodiskdecay_26al_tot += wind_abs_26al_curr
    wind_abs_nodiskdecay_60fe_tot += wind_abs_60fe_curr

    # Calculate sne model yields
    ...
    wind_abs_nodiskdecay_26al_tot *= decay_frac_26al
    wind_abs_nodiskdecay_60fe_tot *= decay_frac_60fe
    ...
    for star_no,star in enumerate(curr_state.cluster):
      if star.disk_alive == True:
        if star.tau_disk >= t_next:
          wind_abs_diskdecay_26al_tot[star_no] = wind_abs_nodiskdecay_26al_tot[star_no]
          wind_abs_diskdecay_60fe_tot[star_no] = wind_abs_nodiskdecay_60fe_tot[star_no]
  ...
  # Stop stellar evolution model for global model
  stellar.stop()

  # Now that simulation has been processed, write data for each star to the dataset
  # This is some of the worst writing out a bunch of arrays into a single database I have ever written
  ...
  for i,star in enumerate(first_cluster):
    ...
    for model in models:
      ...
      for isotope in isotopes:
        ...
        # So begins some of the most hacky decision logic possible, this absolutely sucks
        # Do not use this as a template for anything
        # For 26Al cases
        if isotope == "26al":
          stable_yield = star.mass_27al.value_in(units.MSun)
          if model == "local":
            unstable_yield_nodecay = sim_yield.local_26al[-1,i]
            unstable_yield_decay =   sim_yield.local_26al_final[i]
          if model == "global":
            unstable_yield_nodecay = wind_abs_nodiskdecay_26al_tot[i]
            unstable_yield_decay   = wind_abs_diskdecay_26al_tot[i]
          if model == "sne":
            unstable_yield_nodecay = sne_abs_nodiskdecay_26al_tot[i]
            unstable_yield_decay   = sne_abs_diskdecay_26al_tot[i]
          if model == "local+sne":
            unstable_yield_nodecay = sim_yield.local_26al[-1,i] + sne_abs_nodiskdecay_26al_tot[i]
            unstable_yield_decay =   sim_yield.local_26al_final[i] + sne_abs_diskdecay_26al_tot[i]
          if model == "global+sne":
            unstable_yield_nodecay = wind_abs_nodiskdecay_26al_tot[i] + sne_abs_nodiskdecay_26al_tot[i]
            unstable_yield_decay   = wind_abs_diskdecay_26al_tot[i] + sne_abs_diskdecay_26al_tot[i]
        # For 60Fe cases
        if isotope == "60fe":
          stable_yield = star.mass_56fe.value_in(units.MSun)
          if model == "local":
            unstable_yield_nodecay = sim_yield.local_60fe[-1,i]
            unstable_yield_decay =   sim_yield.local_60fe_final[i]
          if model == "global":
            unstable_yield_nodecay = wind_abs_nodiskdecay_60fe_tot[i]
            unstable_yield_decay   = wind_abs_diskdecay_60fe_tot[i]
          if model == "sne":
            unstable_yield_nodecay = sne_abs_nodiskdecay_60fe_tot[i]
            unstable_yield_decay   = sne_abs_diskdecay_60fe_tot[i]
          if model == "local+sne":
            unstable_yield_nodecay = sim_yield.local_60fe[-1,i] + sne_abs_nodiskdecay_60fe_tot[i]
            unstable_yield_decay =   sim_yield.local_60fe_final[i] + sne_abs_diskdecay_60fe_tot[i]
          if model == "global+sne":
            unstable_yield_nodecay = wind_abs_nodiskdecay_60fe_tot[i] + sne_abs_nodiskdecay_60fe_tot[i]
            unstable_yield_decay   = wind_abs_diskdecay_60fe_tot[i] + sne_abs_diskdecay_60fe_tot[i]
        # Write result of this horrible hacky array to the results array
  ...
  print(df)
  df.to_pickle("global-model-virial-all-sim-ratios.pkl.zst")
